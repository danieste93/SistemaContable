  ...
  state={
    ...
    componentDidMount(){
      setTimeout(function(){ 
        ...
        document.getElementById('maindeleteVentas').classList.add("entradaaddc")
      ...
      Onsalida=()=>{
        document.getElementById('maindeleteVentas').classList.remove("entradaaddc")
        setTimeout(()=>{ 
      ...
      DeleteVenta=(reg)=>{
        ...
        .then(response => {console.log('Deleteventa:', response)

        if(response.message=="error al registrar"){
          alert(`Error en el sistema,${response.error} ,porfavor intente en unos minutos`)
         } 
         else {
...
<div className="maincontacto" id="maindeleteVentas" >
            ...
            <p> Seguro quieres eliminar la Venta N°{this.props.DeleteReg.iDVenta} </p>
...
this.DeleteVenta(this.props.DeleteReg)
}
    }}>
 Aceptar 
  </button>
