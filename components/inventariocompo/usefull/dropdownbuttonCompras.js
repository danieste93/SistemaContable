...
import generarReporteCompras from '../generadorReporteCompras';
import generarReporteComprasExcel from "../generadorReporteExcelCompras";
...
const downloadSellReport = (ventas, state, img) => {
  ...
  generarReporteCompras(ventas, img, state);
};
...
const DropdownButton = ({ arrData, state, img }) => {
  ...
  const handleClickExcel = (event) => {
    if (arrData.length === 0) {
      ...
      generarReporteComprasExcel(arrData, state, img);
    }
  };
