    ...
    componentDidMount(){
     ...
     headers:{
      ...
      if(response.Icons[0]){
        ...
        alert("Error al descargar iconos")
       }
...
let iconosRender = this.state.matrizIcons.map((x,i)=> <div className={`MainContIcon ${borderEdit}`} key={i}>
  <Animate show={this.state.EditMode}>
   <button className=" btn btn-danger botonAddCrom" onClick={()=>{this.setState({itemTodelete:{seleccionado:x,iDgeneral:9999997},modalDelete:true})}}>
  ...
  <div className="contIconoSelect " onClick={()=>{this.props.sendUrl(x)}} >
                
                <img className='iconoCuenta jwPointer' src={x} />
...
<div className="Scrolled">
  ...
  <Animate show={this.state.modalDelete}> 
        ...
        itemTodelete={this.state.itemTodelete}
         mensajeDelete={{mensaje:"Estas seguro quieres eliminar este icono", url:"/public/deleteicon" }}
